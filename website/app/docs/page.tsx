...
export const metadata: Metadata = {
  title: "Documentation: install, quick start and the 13 tools",
  description:
    "One line to install ato-mcp (hosted), sign in from your browser, and explore the 13 tools: cited ATO retrieval, personal context and tax workflows.",
  ...
  return (
    <main className="mx-auto max-w-5xl px-5 pb-24 pt-14">
      ...
      <p className="mt-4 max-w-2xl text-[15px] leading-relaxed text-zinc-500">
        ato-mcp is an MCP server: it plugs into Claude Code, Cursor, Codex or any
        Model Context Protocol host and turns the Australian tax corpus into tools
        your agent can call. One line to install, sign in with your browser, done.
      </p>
      ...
      <section className="mt-14" aria-labelledby="install-h">
        ...
        <div className="mt-5">
          <div className="card p-6">
            ...
            <pre className="code-block mt-4 whitespace-pre-wrap"><code>{`claude mcp add --scope user --transport http ato https://api.ato-mcp.com.au/mcp`}</code></pre>
            <p className="mt-3 text-xs leading-relaxed text-zinc-500">
              Run it, then authenticate from inside your client. Your browser opens to sign in.
              Every other client (Codex, Gemini CLI, Cursor, VS Code, …) is one line too:{" "}
              <Link href="/install" className="text-zinc-900 underline decoration-zinc-300 underline-offset-2 hover:decoration-zinc-900">
                see the install picker
              </Link>
              .
            </p>
          </div>
      ...
      <section className="mt-14" aria-labelledby="connect-h">
        ...
        <div className="card mt-5 p-6">
          <p className="text-sm font-medium">Any MCP host</p>
          <pre className="code-block mt-3 whitespace-pre-wrap"><code>{`{"mcpServers":{"ato":{"url":"https://api.ato-mcp.com.au/mcp"}}}`}</code></pre>
            ...
            <div className="mt-3 grid gap-3 md:grid-cols-2">
              {TOOLS_META.filter((t) => t.group === g).map((t) => (
                <article key={t.name} className="card p-5">
                  ...
                  <pre className="code-block mt-3 whitespace-pre-wrap px-3 py-2 text-[0.6875rem]"><code>{t.example}</code></pre>
                </article>
              ))}
            </div>
          </div>
        ...
        {[
          {
            ...
            title: "GitHub",
            body: "The MCP client is open source (AGPL-3.0). Read the code you run.",
            ext: true,
          },
          { href: "/privacy", title: "Privacy", body: "Generated from the database schema, so it shows exactly what's stored.", ext: false },
        ].map((c) =>
          c.ext ? (
